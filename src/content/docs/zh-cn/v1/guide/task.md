---
title: 任务编排
description: 添加和管理 EasyTidy V1 任务，构建复杂的文件处理工作流。
---
...
4. **设置文件处理规则**：
   - **处理规则**： 根据需要定义详细的文件处理规则，例如文件扩展后缀、正则表达式、特定字符匹配等[规则](/zh-cn/v1/guide/rules/)方式，可以自定义输入，也可以点击输入框旁边的紫色按钮选择系统的预设规则。规则之间需要使用`;`或者`|`来分割，详情参阅[规则配置章节](/zh-cn/v1/guide/rules/)
   - **附加规则**：点击蓝色的`漏斗`图标可以选择在[过滤器章节](/zh-cn/v1/guide/filter/)添加的过滤规则，并且与该任务关联，与处理规则为"且"的关系
...
8. **保存并应用**：
   - 完成所有设置后，点击"保存"按钮来确认配置，使新任务生效。此时可以在[自动化](/zh-cn/v1/guide/automation/)页面对此任务进行自动化配置。
